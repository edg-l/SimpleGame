...
Player *player_create(int accel, int max_speed, int max_hp, int max_mana) {
    ...
    p->hp_bar =
        pb_create(screen.w / 3, 25, COLOR_WHITE, COLOR_WHITE, COLOR_RED);
    p->mana_bar =
        pb_create(screen.w / 3, 25, COLOR_WHITE, COLOR_WHITE, COLOR_BLUE);
...
int player_collide(Player *p, Point speed, Tilemap *t, Rect *rects,
                   Rect *intersection) {
    Rect a = p->rect;
    a.x += speed.x;
    a.y += speed.y;
    for (int y = a.y; y < a.y + a.h + t->tileSize / 2; y += t->tileSize / 2) {
        for (int x = a.x; x < a.x + a.w + t->tileSize / 2;
             x += t->tileSize / 2) {
