...
void util_quit() { free(prev_keyboard_status); }

Color util_color(unsigned char r, unsigned char g, unsigned char b,
                 unsigned char a) {
    return (Color){r, g, b, a};
}

Rect util_rect(int x, int y, int w, int h) { return (Rect){x, y, w, h}; }

Point util_point(int x, int y) { return (Point){x, y}; }

Point util_mouse_pos() { return mouse_pos; }
...
Rect util_screen() {
    return util_rect(0, 0, settings_get_int("window_width"),
...
int util_is_keypress(int code) { return keyboard_status[code]; }

Uint32 util_tick() { return SDL_GetTicks(); }

int util_tick_passed(Uint32 a) { return SDL_TICKS_PASSED(util_tick(), a); }
...
double util_delta_time() {
    return (double)((now_time - last_time) * 1000) /
           SDL_GetPerformanceFrequency();
}

double util_fps() { return 1 / (util_delta_time() / 1000); }
...
Point util_world_to_tilemap(Tilemap *t, Point world) {
    return util_point(floor(world.x / (float)t->tileSize),
